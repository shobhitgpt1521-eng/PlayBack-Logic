...
VideoPlayer::VideoPlayer(QObject* parent): QObject(parent){
    ...
    connect(posTimer,&QTimer::timeout,this,[this]{
        if(!pipeline) return;
        gint64 pos=0,dur=0;
        if(gst_element_query_position(pipeline,GST_FORMAT_TIME,&pos)) emit positionNs(pos);
        if(gst_element_query_duration(pipeline,GST_FORMAT_TIME,&dur)) emit durationNs(dur);
    });
    posTimer->start(200);
}
...
void VideoPlayer::setWindowHandle(quintptr wid){ winHandle=wid; bindOverlay(); }
...
bool VideoPlayer::open(const QString& path){
    ...
    videosink = mk("glimagesink"); if(!videosink) videosink=mk("ximagesink"); if(!videosink) videosink=mk("autovideosink");
    if(!pipeline||!filesrc||!demux||!parser||!decoder||!vconv||!videosink){ emit errorText("Failed to create GStreamer elements"); teardown(); return false; }
    ...
    if(!gst_element_link_many(parser,decoder,vconv,videosink,nullptr)){ emit errorText("Link failed: parser→decoder→vconv→sink"); teardown(); return false; }
    if(!gst_element_link(filesrc,demux)){ emit errorText("Link failed: filesrc→qtdemux"); teardown(); return false; }
    ...
    g_signal_connect(demux,"pad-added",
        G_CALLBACK(+[](GstElement*,GstPad* pad,gpointer user){
            ...
            bool isVideo=false;
            if(caps){ const gchar* n=gst_structure_get_name(gst_caps_get_structure(caps,0)); if(n&&g_str_has_prefix(n,"video/")) isVideo=true; gst_caps_unref(caps); }
            if(!isVideo) return;
    ...
    if(gst_element_get_state(pipeline,nullptr,nullptr,3*GST_SECOND)!=GST_STATE_CHANGE_SUCCESS){
        emit errorText("Preroll failed"); teardown(); return false;
    }

    // ensure desired rate is applied (e.g., after file switch)
    setRate(rate_);

    // push duration once
    gint64 dur=0; if(gst_element_query_duration(pipeline,GST_FORMAT_TIME,&dur)) emit durationNs(dur);
    return true;
}
...
void VideoPlayer::stop(){  if(pipeline){ gst_element_set_state(pipeline,GST_STATE_NULL); } }

bool VideoPlayer::seekNs(qint64 t_ns){
    if(!pipeline) return false;
    gboolean ok = gst_element_seek(pipeline, rate_, GST_FORMAT_TIME,
        (GstSeekFlags)(GST_SEEK_FLAG_FLUSH | GST_SEEK_FLAG_ACCURATE | GST_SEEK_FLAG_KEY_UNIT),
...
bool VideoPlayer::setRate(double r){
    if(r==0.0) r=1.0;              // avoid pause via 0
    rate_=r;
    if(!pipeline) return true;
    // rate change via flushing seek at current position
...
gboolean VideoPlayer::bus_cb(GstBus*,GstMessage* msg,gpointer s){
    ...
    case GST_MESSAGE_ERROR:{
        GError* err=nullptr; gchar* dbg=nullptr; gst_message_parse_error(msg,&err,&dbg);
        if(self) self->errorText(QString::fromUtf8(err?err->message:"GStreamer error"));
        if(dbg) qWarning("GST DEBUG: %s", dbg);
        g_clear_error(&err); g_free(dbg);
        break;}
    case GST_MESSAGE_EOS: if(self) emit self->eos(); break;
    default: break;
    }
    return TRUE;
}
